...
export async function POST(request: Request) {
  try {
    const { guess, dayNumber } = await request.json();

    if (typeof guess !== "string" || typeof dayNumber !== "number") {
    ...
    return NextResponse.json({
      isCorrect,
      trackTitle: isCorrect ? dailyTrack.title : undefined,
      trackImage: isCorrect ? dailyTrack.md5_image : undefined,
    });
  } catch (error) {
    console.error(
      "Error in validate-guess API:",
